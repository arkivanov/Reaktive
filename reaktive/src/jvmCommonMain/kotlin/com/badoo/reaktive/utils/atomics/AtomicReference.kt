...
internal actual inline fun <T> AtomicReference<T>.getValue(): T = get()

internal actual inline fun <T> AtomicReference<T>.setValue(value: T) {
    set(value)
}

@Suppress("EXTENSION_SHADOWED_BY_MEMBER") // The signature of the original method is same
internal actual inline fun <T> AtomicReference<T>.compareAndSet(expectedValue: T, newValue: T): Boolean =
    this.compareAndSet(expectedValue, newValue)

internal actual inline fun <T> atomic(initialValue: T): AtomicReference<T> = AtomicReference<T>(initialValue)
